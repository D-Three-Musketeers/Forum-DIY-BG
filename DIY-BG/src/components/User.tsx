...
const User = () => {
  ...
  const fetchUserPosts = async () => {
    ...
    setPostsLoding(true);
    try {
      // Optimized query - only fetch posts by this user
      ...
      if (postsSnapshot.exists()) {
        ...
        const userPosts = Object.entries(postsObj).map(([postId, post]) => {
          const p = post as any;
          return {
            id: postId,
            ...p,
            ...
            commentCount: commentCountMap[postId] || 0, // Get from precomputed map
          };
        });
