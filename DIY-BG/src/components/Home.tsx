  ...
  const filteredAndSortedPosts = useMemo(() => {
    ...
    switch (sortMethod) {
      ...
      default:
        return [...filteredPosts].sort(
          (a, b) =>
            new Date(b[1].timestamp || 0).getTime() -
            new Date(a[1].timestamp || 0).getTime()
        );
    }
          ...
          <div className="col-md-6 mb-3 mb-md-0">
            <div className="d-flex gap-2 flex-wrap">
              {[1, 2, 3, 4].map((i) => (
                <button
                  key={i}
                  className="btn btn-outline-primary placeholder col-2"
                ></button>
              ))}
            </div>
          </div>
          ...
          <div className="d-flex justify-content-end gap-2">
            {["users", "posts", "comments"].map((stat) => (
              ...
              >
                <small className="card-title">
                  {t(
                    `home.total${stat.charAt(0).toUpperCase() + stat.slice(1)}`
                  )}
                </small>
                <h6 className="card-text mb-0">
                  {stats.loadingStats ? (
